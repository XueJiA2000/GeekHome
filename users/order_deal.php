<?php
	...
	if($act=="comment"){//执行添加
		...
		$sql="insert into ite_product_comment(content,upfile1,upfile2,upfile3,username,goods_name,goods_price) values('".$content."','".@$fileName."','".@$fileName."','".@$fileName."','".$_SESSION["username"]."','".$goods_name."','".$goods_price."')";
		//exit($sql);
		...
		if($result && $affect_rows){
			$Id=$_GET["Id"];
			$sql = "delete from `ite_product_comment` where Id=".$Id."";
			$result=mysql_query($sql);//执行SQL语句
